...
class Parser:  # PASSED VERSION 1
  ...
  def advance(self):  # PASSED
    ...
    It should be called only if has_more_commands() returns true.  Initially, the current
    command is None.'''
  ...
  def command_type(self):
    '''Returns a constant representing the type of the current command. C_ARITHMETIC is
    returned for all the arithmetic/logical commands'''
  ...
  def arg1(self):
    '''Returns the first argument of the _current_command.  In the case of C_ARITHMETIC,
    the command itself (add, sub, etc.) is returned. Should not be called if the current
    command is C_RETURN.'''
    if self.command_type == C_RETURN:
      raise ValueError("arg1 should not be called if command type is C_RETURN")
...
# _test_open_file("test.vm")
# _test_parser("test.vm")
